...
import os
import pdb
from .. import MarkovTextGenerator2Node

#update to reflect how the 2 part generator uses pairs instead of strings

def mocked_randint_one(a, b):
    return 1
def mocked_randint_zero(a, b):
    return 0
def mocked_randint_seq(a, b, sequence = [1,1,2,3,0]):
...
class markov_test(unittest.TestCase):
    ...
    def test_simple_add_gen(self):
        chain1 = MarkovTextGenerator2Node(None)
        ...
        node_one = chain1.chain[(chain1.start,"Hello")]
        node_two = chain1.chain[("Hello","there!")]

        self.assertEqual(node_one.state[1], "Hello", msg="Node state incorrect")
        self.assertEqual(node_two.state[1], "there!", msg="Node state incorrect")
        #self.assertEqual(node_one.count, 1, msg="Node count incorrect")
        #self.assertEqual(node_two.count, 1, msg="Node count incorrect")

        self.assertEqual(node_one.nextstate[0].state[1], "there!", msg="Next state incorrect")
        self.assertEqual(node_two.nextstate[0].state[1], chain1.end, msg="Next state incorrect")
        result = chain1.generate(50, mocked_randint_zero)
        self.assertEqual(result, "Hello there!")

    def test_complex_add_gen(self):
        chain = MarkovTextGenerator2Node(None)
    ...
    def test_file_io(self):
        chain = MarkovTextGenerator2Node("test.txt")
